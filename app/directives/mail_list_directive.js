...
angular.module('emailClientApp').directive('outboxList', function($rootScope, model, $location, mailUtils) {
  return {
    ...
    link: function(scope, element) {
			scope.text = "loading...";
			scope.dataLoading = true;
			var table = element.find("#outboxTable");
			var lastEmail = {};
      ...
      $rootScope.$on('updateOutbox', function (event, newMail) {
          var newRow = angular.element(mailUtils.prepareSentEmail(newMail));
      ...
      element.bind('click', function(event) {
        ...
        if(clickedEl.tagName === 'TR') { // When it's the parent
            $location.path("outbox/" + clickedEl.id); // Change location to email view
            scope.$apply();
        }
      });
